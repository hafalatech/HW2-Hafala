...
void testChangeRequestedTimeForShort()                 // HW2 - Lotem - NOT SURE HOW TO RUN THIS TEST HERE...
{                                           //because we need to check the trial num
    int id = fork();
    int status;
    if (id > 0) {
    ...
    paramIn.number_of_trials = expected_trials;
    ...
    //change requested_time fail because of different number_of_trials
    paramIn.requested_time = new_expected_requested_time;
    paramIn.number_of_trials = 40;
    assert(sched_setparam(id, &paramIn) ==-1);
...
void testScheduleOtherOverOVERDUEBecauseOfTime2()
{
    ...
    {
        doLongTask(); //hold on for a second , let the father make the sons short
        ...
        if (id > 0) {
            ...
            else
            {
                struct sched_param param2;
                param2.sched_priority = 1;
                printf("making id2 OTHER\n");
                sched_setscheduler(id2, 0, &param2);             // regular SCHED_OTHER
                printf("making id SHORT\n");
                sched_setscheduler(id, SCHED_SHORT, &param1);         // SHORT_OVERDUE process
            }
            wait(&status);
            wait(&status);
            ...
            doLongTask();
            doLongTask();
            printf("\tSCHED_SHORT overdue son finished\n");
            _exit(0);
        }
        _exit(0);
    }
...
void testMakeShort()
{
        ...
        inputParam.requested_time = expected_requested_time;
        inputParam.number_of_trials = expected_number_of_trials;
        ...
        assert(outputParam.requested_time == expected_requested_time);
        assert(outputParam.number_of_trials == expected_number_of_trials);
        int i;
        doMediumTask();
        assert(sched_getparam(thisId, &outputParam) == 0);
